...
def fetch_raw_transactions(driver, month, year):
    ...
    time.sleep(1) # If we go too fast first rows are mangled. Not sure why.
...
def login(username, password):
    ...
    try:
        WebDriverWait(driver, 30).until_not(lambda d: driver.find_element(By.ID, "continueBtn").click() or 1)
